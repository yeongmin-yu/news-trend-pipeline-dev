...
2. `provider`(공급원) 값을 함께 넘겨 언어별 전처리를 분기한다.
...
if provider == "naver":
    ...
    text = re.sub(r"[^A-Za-z0-9\s]", " ", text)       # 영숫자·공백만 유지
```
...
| 입력 | provider | 출력 |
|------|----------|------|
| `<b>AI</b> 2026 혁신!` | `naver` | `혁신` |
| `Samsung released Galaxy https://t.co/abc` | `newsapi` | `Samsung released Galaxy` |
| `[+1500 chars]...더 읽기` | `naver` | `` (잔여 없음) |
| `&lt;p&gt;인공지능 반도체&lt;/p&gt;` | `naver` | `인공지능 반도체` |

> **주의**: Naver provider는 한글만 남기기 때문에 숫자·영문이 모두 제거된다. 숫자 정보가 중요한 도메인이라면 이 정책을 재검토해야 한다.

---

## 4. 토큰화 (tokenize)

**파일**: `preprocessing.py:282`

`clean_text()` 호출 후 provider에 따라 두 경로로 분기한다.

### 4-1. 한국어 경로 (`provider == "naver"`)
...
- Kiwi 없으면 공백 분리 fallback으로 동작한다.

### 4-2. 영어 경로 (그 외 provider)
...
- **spaCy** `en_core_web_sm` 모델로 명사(NOUN)·고유명사(PROPN)만 추출한다.
- 숫자만으로 구성된 토큰, 길이 1 이하 토큰은 제거한다.
- spaCy 없으면 공백 분리 fallback으로 동작한다.
...
### 영어 불용어 (`ENGLISH_STOPWORDS`)

관사, 전치사, 대명사, 뉴스 도메인 특화 단어(`reuters`, `news`, `http`, `www`)를 포함한다.

```python
# 대표 예시
{"a", "an", "the", "of", "in", "to", "and", "is", "are",
 "reuters", "news", "http", "https", "www", ...}
```
...
- 토큰 길이 > 1 (단자 제거)
- 숫자만으로 구성된 토큰 제거 (`token.isdigit()`)
- 영어 토큰에서 알파벳이 없는 토큰 제거 (`re.search(r"[a-z]", ...)`)
...
|------|------|
| 역할 | 영어 품사 태깅 및 명사 추출 |
| 사용 이유 | 산업 표준, 빠른 추론 속도, 다양한 모델 지원 |
| 사용 모델 | `en_core_web_sm` (기본, 환경변수로 변경 가능) |
...
품사 태깅(`tagger`)만 사용하고 나머지는 비활성화해 처리 속도를 최적화했다.

**spaCy Universal POS 태그**
...
|------|-----------|-------|
| 대상 언어 | 한국어 | 다국어 (주로 영어) |
| 처리 방식 | 형태소 분석 | 토큰화 + 품사 태깅 |
| 사용자 사전 | 동적 추가 가능 | 별도 커스터마이징 필요 |
| 속도 | 빠름 | 빠름 |
| 모델 크기 | ~수십 MB | `sm` ~12MB, `lg` ~800MB |
| Spark UDF 호환 | 가능 (단, 워커별 로드) | 가능 (단, 워커별 로드) |

---

## 9. Fallback 전략

라이브러리가 설치되지 않은 환경이나 로드 실패 시에도 전처리가 동작하도록 단계별 fallback이 구현되어 있다.
...
영어 경로:
  spaCy 사용 가능? ── YES → spaCy 품사 태깅 + 명사 필터
        │
        NO ↓
  공백 분리 + 불용어 제거 + 숫자 제거
```

Fallback은 품질이 낮아지지만 파이프라인 자체는 중단되지 않는다. `ImportError`는 모듈 로드 시점에 처리하고, 로드 실패는 `get_kiwi()` / `get_spacy_nlp()` 내에서 `None` 반환으로 처리한다.
...
- `get_kiwi()` / `get_spacy_nlp()`는 `@lru_cache`로 캐싱되어 워커 프로세스당 한 번만 초기화된다.
- 각 Spark 워커에서 독립적으로 라이브러리가 로드되므로 워커 이미지에 의존성이 포함되어야 한다.
...
#### 영어 뉴스 lemmatization 적용
현재 영어 경로에서 spaCy의 lemmatizer를 비활성화했기 때문에 `running`, `runs`, `ran`이 별개의 토큰으로 집계된다. lemmatizer를 활성화하거나 별도 정규화를 추가하면 집계 정밀도가 높아진다.
...
# 개선 후 (성능 트레이드오프 있음)
disable=["parser", "ner", "textcat"]  # lemmatizer 활성화
```
...
def tokenize_pandas_udf(texts: pd.Series, providers: pd.Series) -> pd.Series:
    return pd.Series([
        tokenize(text, provider)
        for text, provider in zip(texts, providers)
    ])
```

#### 다국어 확장
현재 `naver` 이외는 모두 영어 경로로 처리된다. 중국어, 일본어 등 추가 언어 지원이 필요한 경우 provider 분기를 확장하고 해당 언어용 형태소 분석기를 추가해야 한다.
...
#### Naver 기사의 영문 키워드 처리
현재 Naver 경로는 한글만 남기기 때문에 영문 고유명사(`AI`, `ChatGPT`, `GPU`)가 모두 제거된다. 한글+영문 혼재 처리 로직 추가를 검토할 수 있다.
