...
| `compound_dictionary_dag` | 복합명사 후보 추출 | 최근 기사 window batch | PostgreSQL `news_raw` | `compound_noun_candidates` | 15분 |
| `compound_candidate_auto_review_dag` | 후보 자동 평가 및 high confidence 자동승인 | 후보 batch review | `compound_noun_candidates`, Naver Web Search API | `auto_score`, `auto_evidence`, `compound_noun_dict` | 15분 |
...
default_args = {
    ...
    # 이전 실행의 성공 여부에 현재 실행이 묶이지 않도록 한다.
    # 특정 실행 실패가 이후 스케줄 전체를 막지 않게 해 운영 안정성을 높인다.
    # 단, 이전 실행과 현재 실행이 같은 데이터 구간을 처리하지 않도록 DAG별 수집 window 계산은 별도로 명확해야 한다.
...
| max_active_runs | 1. 같은 DAG가 동시에 여러 개 실행되지 않도록 제한한다|
...
- `max_active_runs=1`
  - 같은 DAG가 동시에 여러 개 실행되지 않도록 제한한다.
  - 동일 수집 구간, 동일 dead letter batch, 동일 후보 추출 window가 병렬로 처리되는 상황을 줄인다.
  - 이는 멱등성을 직접 보장한다기보다, 중복 처리 가능성을 낮추는 동시성 제어 장치다.
- `catchup=False`
  - DAG가 중지되었다가 재개될 때 과거 스케줄을 한꺼번에 실행하지 않는다.
  - 오래된 수집 구간이 대량으로 재실행되면서 중복 적재의 가능성을 줄인다.
...
| --- | --- |
| schedule interval | `*/15 * * * *` |
| 근거 | 수집 주기와 맞춰 최근 2시간 window를 촘촘하게 갱신 |
...
| --- | --- |
| schedule interval | `*/15 * * * *` |
| 근거 | 후보 추출 주기와 맞춰 새 후보를 촘촘하게 평가하되, 재평가 캐시와 max_active_runs로 중복 처리를 제한 |
| start_date | 예: `2026-01-01` |
| retries | 1 |
| retry_delay | 10 |
| catchup | False. 과거 누락 실행 스킵|
