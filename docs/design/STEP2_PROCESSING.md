...
```mermaid
flowchart LR
    A["Kafka<br/>news_topic"] --> B["Spark Structured Streaming"]
    B --> C["JSON parsing"]
    C --> D["title + summary 결합"]
    D --> E["tokenize(text, domain)"]
    E --> F["기사별 keyword 집계"]
    F --> G["10분 window trend 집계"]
    F --> H["대표 keyword pair 집계"]
    C --> I["stg_news_raw"]
    F --> J["stg_keywords"]
    G --> K["stg_keyword_trends"]
    H --> L["stg_keyword_relations"]
    I --> M["news_raw"]
    J --> N["keywords"]
    K --> O["keyword_trends"]
