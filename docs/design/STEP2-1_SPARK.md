...
```mermaid
flowchart LR
    A["Kafka readStream"] --> B["from_json(ARTICLE_SCHEMA)"]
    B --> C["tokens 생성"]
    C --> D["news_raw staging write"]
    C --> E["article keyword 집계"]
    E --> F["keyword trend 집계"]
    E --> G["keyword relation 집계"]
    D --> H["upsert news_raw"]
    E --> I["upsert keywords"]
    F --> J["upsert keyword_trends"]
    G --> K["upsert keyword_relations"]
```
