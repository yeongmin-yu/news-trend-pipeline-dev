...
class Settings:
    ...
    kafka_retries: int = int(os.getenv("KAFKA_RETRIES", "1"))
    kafka_max_in_flight: int = int(os.getenv("KAFKA_MAX_IN_FLIGHT", "1"))
    kafka_request_timeout_ms: int = int(os.getenv("KAFKA_REQUEST_TIMEOUT_MS", "10000"))
    kafka_max_block_ms: int = int(os.getenv("KAFKA_MAX_BLOCK_MS", "5000"))
    kafka_flush_timeout_seconds: int = int(os.getenv("KAFKA_FLUSH_TIMEOUT_SECONDS", "10"))
    ...
    spark_preprocess_partitions: int = int(os.getenv("SPARK_PREPROCESS_PARTITIONS", "0"))
    spark_jdbc_batch_size: int = int(os.getenv("SPARK_JDBC_BATCH_SIZE", "5000"))
    spark_jdbc_num_partitions: int = int(os.getenv("SPARK_JDBC_NUM_PARTITIONS", "0"))
