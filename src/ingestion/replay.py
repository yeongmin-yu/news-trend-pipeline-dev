...
def run_replay(dry_run: bool = False) -> dict[str, int] | None:
    ...
    for record in records:
        ...
        try:
            ...
            logger.error("DLQ payload validation 실패: %s payload=%s", exc, payload)
        ...
        if attempt > MAX_RETRY_ATTEMPTS:
            logger.error("영구 실패 처리: url=%s attempt=%d max=%d", url, attempt - 1, MAX_RETRY_ATTEMPTS)
            permanent_failed.append({**record, "attempt": attempt - 1, "permanent_fail_at": utc_now_iso()})
            permanent_fail_count += 1
            continue
        ...
        try:
            ...
            future = producer.producer.send(settings.kafka_topic, value=message, key=partition_key)
            ...
            logger.warning("재전송 실패: url=%s error=%s attempt=%d/%d", url, exc, attempt, MAX_RETRY_ATTEMPTS)
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="실제 전송 없이 재전송 대상만 확인합니다.")
